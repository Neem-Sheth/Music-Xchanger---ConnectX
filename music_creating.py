...
class MusicTransferApp:
    def __init__(self, master):
        ...
        self.received_filename = "received_music.mp3"  # Name for the file to save received music
    ...
    def create_widgets(self):
        ...
        self.select_file_button = Button(self.master, text="Select Music File", command=self.select_file,
                                         font="arial 11")
        ...
        self.receive_button = Button(self.master, text="Receive Music File", command=self.receive_file,
                                     font="arial 11")
        self.receive_button.place(x=300, y=230)

        # Display local IP address
        self.local_ip_label = Label(self.master,
                                    text=f"My Local IP is: {socket.gethostbyname(socket.gethostname())}",
                                    bg="antiquewhite", fg="black", font="arial 10 bold")
        ...
        if target_ip and self.filename:
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s, open(self.filename, 'rb') as file:
                    ...
                    if os.path.exists(self.received_filename):
                        with open(self.received_filename, 'rb') as received_file:
                            while file_data := received_file.read(1024):
                                s.send(file_data)
        ...
        if filename1:
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    ...
                    s.listen(1)
                    conn, addr = s.accept()
                    with conn, open(self.received_filename, 'ab') as file:
